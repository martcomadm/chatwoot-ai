...
export class OperationsConfigStore {
  ...
  async deleteMasterAgent(agentId, actor="inspector-admin"){
    const id = Number(agentId);
    if(!Number.isFinite(id) || id <= 0) throw new Error("ID de asesor inválido");

    const memberships = ["weekday","saturday","sunday"].filter(group =>
      (this.state.groups?.[group] || []).some(item => Number(item.id) === id)
    );

    const exceptionMemberships = Object.entries(this.state.exceptions || {})
      .filter(([,agents]) => (agents || []).some(item => Number(item.id) === id))
      .map(([date]) => date);

    if(memberships.length || exceptionMemberships.length){
      const where = [
        memberships.length ? `turnos: ${memberships.join(", ")}` : "",
        exceptionMemberships.length ? `excepciones: ${exceptionMemberships.join(", ")}` : ""
      ].filter(Boolean).join(" · ");
      throw new Error(`No se puede eliminar del catálogo porque todavía está asignado en ${where}. Quítalo primero de esas rotaciones.`);
    }

    const before = this.allAgents().find(item => Number(item.id) === id) || null;
    this.state.agents = (this.state.agents || []).filter(item => Number(item.id) !== id);
    this.state.updated_at = nowIso();
    this.audit("master_agent_deleted", actor, {agent_id:id,agent:before});
    await this.persist();
    return this.snapshot();
  }
