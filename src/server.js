...
import { HandoffRotationStore, HandoffRouter } from "./handoff/index.js";
...
try {
  ...
  const handoffRouter = new HandoffRouter({ config, store: handoffRotation, chatwoot });
  ...
  app.use(createRouter({ config, memories, buffer, inspectorEvents, handoffRotation }));

  app.listen(config.port, "0.0.0.0", () => {
    console.log(`MARTCOM AI V3.2.2 escuchando en puerto ${config.port}`);
    ...
    console.log(`Inspector: /inspector · versión 1.2.1`);
    console.log(`Advisor Affinity: presentación y handoff usan el mismo asesor reservado`);
  ...
  console.error("No se pudo iniciar MARTCOM AI V3.2.2:", error);
  process.exit(1);
}
