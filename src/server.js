...
try {
  ...
  const processor = new ConversationProcessor({ config, chatwoot, labels, memories, agentRotation, ai });
  const buffer = new MessageBuffer(config.ai.bufferMs, (id, snapshot) => processor.process(id, snapshot));

  const app = express();
  app.use(express.json({ limit: "4mb" }));
  app.use(createRouter({ config, memories, buffer }));

  app.listen(config.port, "0.0.0.0", () => {
    console.log(`MARTCOM AI V3.0.0 escuchando en puerto ${config.port}`);
  ...
  console.error("No se pudo iniciar MARTCOM AI V3.0.0:", error);
  process.exit(1);
}
