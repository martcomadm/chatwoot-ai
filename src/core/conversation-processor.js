...
if(memory.sales_cycle?.authorized&&this.workflow){if(documentPatch?.ine_recibida===true&&memory.operations?.onboarding_next==="ine"){const incoming=batch.flatMap(m=>(m.attachments||[]).map(a=>({...a,file_name:a.file_name||a.filename||a.name||"ine.jpg",file_type:a.file_type||a.content_type||"image"})));if(incoming.length){for(const a of incoming){a.file_name=`ine_${a.file_name}`;}conversation.messages=[...(conversation.messages||[]),{id:`vision-${Date.now()}`,created_at:Date.now()/1000,attachments:incoming}];}}const result=await ensureAuthorizedSale({workflow:this.workflow,memories:this.memories,inspectorEvents:this.inspectorEvents,conversationId,conversation,memory});memory=this.memories.get(conversationId);await this.record(conversationId,"authorized_sale_workflow",{sale_id:result.sale?.sale_id,status:result.sale?.status,documents_complete:result.sale?.documents?.complete,missing:result.sale?.documents?.missing||[]});}
