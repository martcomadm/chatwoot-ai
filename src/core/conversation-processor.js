  ...
  async process(conversationId, snapshot) {
    ...
    if (pendingHandoffMemory.handoff?.status === "pending" && !pendingHandoffMemory.handoff?.completed) {
      ...
      if (retryResult.status === "completed") {
        console.log(JSON.stringify({ event: "handoff_retry_completed", version: "3.3.2.1", conversationId }));
        return;
      }
    }
    ...
    const negationScope = resolveNegationScope(combinedText);
    const judgment = analyzeJudgment(combinedText, memoryWithIntent);
    if (negationScope.status === "ambiguous") {
      judgment.shouldHandoff = false;
      judgment.handoffReason = null;
      judgment.directAnswer = negationScope.clarification;
      judgment.question = { type:"clarify_interest", answerKey:null };
      judgment.objection = { type:"ambiguous_negation", severity:"high" };
      judgment.interrupt = { active:true,type:"ambiguous_negation",priority:"high",resume_planner:false };
      judgment.patch = judgment.patch || {};
      judgment.patch.judgment = {
        ...(judgment.patch.judgment || {}),
        ambiguous_negation:true,
        clarification_pending:true,
        clarification_type:"service_interest"
      };
      await this.record(conversationId, "negation_scope_ambiguous", {
        text: joined,
        reason: negationScope.reason,
        confidence: negationScope.confidence
      });
    }
    ...
    for (const event of reliability.slotEvents || []) await this.record(conversationId, event.type || "slot_governed", event);
    ...
    for (const event of judgment.events) await this.record(conversationId, event.type, event);
    ...
    const preferredDirectAnswer = judgment.directAnswer || orchestration.directAnswer;
    memory = mergeMemory(memoryWithIntent, llmPatch, reliability.patch, judgment.patch, { orchestration: { direct_request: judgment.question || orchestration.directRequest, direct_answer: preferredDirectAnswer }, judgment: judgment.patch.judgment });
    ...
    if (judgment.shouldHandoff) {
      ...
      const message = judgment.humanPreference
        ? "Entiendo. Para no hacerte pasar por más preguntas, voy a dejar tu caso listo para que continúe directamente un asesor."
        : judgment.question?.type === "price"
          ? "Entiendo que quieras conocer el costo antes de compartir más datos. Prefiero que un asesor revise contigo las opciones y precios para que tengas claridad antes de continuar."
          : "Ese punto prefiero que te lo explique directamente un asesor para darte información precisa.";
      ...
      console.log(JSON.stringify({ event: "judgment_handoff", version: "3.3.2.1", conversationId, messageIds, reason: judgment.handoffReason }));
      return;
    }
    ...
    if (Number(memory.experiencia?.frustration_score || 0) >= 2) {
      ...
      console.log(JSON.stringify({ event: "frustration_handoff", version: "3.3.2.1", conversationId, messageIds }));
      return;
    }
    ...
    if (orchestration.shouldHandoffB2B || memory.intent?.id === "PROVEEDOR") {
      ...
      console.log(JSON.stringify({ event: "b2b_handoff", version: "3.3.2.1", conversationId, messageIds }));
      return;
    }
    ...
    if (reason) {
      ...
      console.log(JSON.stringify({ event: "handoff", version: "3.3.2.1", conversationId, messageIds, reason, sources: snapshot.sources, memory }));
      return;
    }
    ...
    await this.memories.markProcessedMany(conversationId, messageIds);
    console.log(JSON.stringify({ event: "processed", version: "3.3.2.1", conversationId, messageIds, sources: snapshot.sources, planner, labels: currentLabels, memory: this.memories.get(conversationId) }));
  }
}
