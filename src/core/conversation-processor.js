...
export class ConversationProcessor {
  ...
  async process(conversationId, snapshot) {
    ...
    const reliability = analyzeReliability(combinedText, memoryWithIntent, fastPatch);
    ...
    memory = mergeMemory(memoryWithIntent, llmPatch, reliability.patch);
    ...
    if (Number(memory.experiencia?.frustration_score || 0) >= 2) {
      ...
      console.log(JSON.stringify({ event: "frustration_handoff", version: "3.1.2", conversationId, messageIds }));
      return;
    }
    ...
    if (reason) {
      ...
      console.log(JSON.stringify({ event: "handoff", version: "3.1.2", conversationId, messageIds, reason, sources: snapshot.sources, memory }));
      return;
    }
    ...
    if (decision.handoff || planner.action === "transferir" || planner.action === "transferir_orientacion_fallecimiento") {
    ...
    await this.memories.markProcessedMany(conversationId, messageIds);
    console.log(JSON.stringify({ event: "processed", version: "3.1.2", conversationId, messageIds, sources: snapshot.sources, planner, labels: currentLabels, memory: this.memories.get(conversationId) }));
  }
}
