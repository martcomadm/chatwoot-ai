...
const conversation=await this.chatwoot.getConversation(conversationId);const webhookMessages=[...(snapshot?.webhookMessages?.values?.()||[])];if(webhookMessages.length){const byId=new Map(messagesOf(conversation).filter(m=>m?.id).map(m=>[String(m.id),m]));for(const wm of webhookMessages){if(!wm?.id)continue;const existing=byId.get(String(wm.id));const wa=Array.isArray(wm.attachments)?wm.attachments:[];if(existing&&wa.length)existing.attachments=wa;else if(!existing)conversation.messages=[...(conversation.messages||[]),wm];}}if(Number(conversation?.inbox_id||conversation?.inbox?.id)!==Number(this.config.chatwoot.inboxId))return;const assigneeId=Number(conversation?.meta?.assignee?.id||conversation?.assignee?.id||0);if(assigneeId&&assigneeId!==Number(this.config.chatwoot.agentId))return;let currentLabels=conversation?.labels||[];if(currentLabels.some(label=>stopLabels.has(label)))return;const batch=batchFrom(conversation,snapshot,this.memories,conversationId);if(!batch.length)return;await this.record(conversationId,"attachment_pipeline_debug",{snapshot_ids:(snapshot.ids||[]).map(String),webhook:[...(snapshot.webhookMessages?.values?.()||[])].map(m=>({id:String(m?.id||""),attachments:Array.isArray(m?.attachments)?m.attachments.length:0,keys:(m?.attachments||[]).map(x=>Object.keys(x||{})),type:(m?.attachments||[]).map(x=>String(x?.file_type||x?.content_type||"")),has_url:(m?.attachments||[]).map(x=>Boolean(x?.data_url||x?.download_url||x?.file_url||x?.url))})),batch:batch.map(m=>({id:String(m?.id||""),attachments:Array.isArray(m?.attachments)?m.attachments.length:0,keys:(m?.attachments||[]).map(x=>Object.keys(x||{})),type:(m?.attachments||[]).map(x=>String(x?.file_type||x?.content_type||"")),has_url:(m?.attachments||[]).map(x=>Boolean(x?.data_url||x?.download_url||x?.file_url||x?.url))}))});const messageIds=batch.map(m=>m.id);const combinedText=batch.map(m=>m.content||"").filter(Boolean).join("\n").trim();if(!combinedText&&!batch.some(hasAttachments)){await this.memories.markProcessedMany(conversationId,messageIds);return;}
...
if(memory.sales_cycle?.authorized&&this.workflow){const expectedDocument=memory.operations?.onboarding_next||memory.operations?.onboarding_last_requested||null;const incoming=batch.flatMap(m=>(m.attachments||[]).map(a=>({...a})));if(incoming.length&&expectedDocument==="ine"){for(const a of incoming){a.file_name=`ine_${a.file_name||a.filename||a.name||"documento"}`;a.file_type=a.file_type||a.content_type||"image";}conversation.messages=[...(conversation.messages||[]),{id:`incoming-${Date.now()}`,created_at:Date.now()/1000,attachments:incoming}];await this.record(conversationId,"document_attachment_injected",{expected:expectedDocument,count:incoming.length,vision_ine:Boolean(documentPatch?.ine_recibida)});}const result=await ensureAuthorizedSale({workflow:this.workflow,memories:this.memories,inspectorEvents:this.inspectorEvents,conversationId,conversation,memory});memory=this.memories.get(conversationId);await this.record(conversationId,"authorized_sale_workflow",{sale_id:result.sale?.sale_id,status:result.sale?.status,documents_complete:result.sale?.documents?.complete,missing:result.sale?.documents?.missing||[]});await this.record(conversationId,"document_sale_debug",{expected:expectedDocument,incoming_count:incoming.length,files:(result.sale?.documents?.files||[]).map(x=>({type:x?.type||null,message_id:x?.message_id||null,attachment_id:x?.attachment_id||null})),missing:result.sale?.documents?.missing||[]});}
