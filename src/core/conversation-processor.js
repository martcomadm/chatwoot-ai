...
export class ConversationProcessor {
  ...
  async performAutoHandoff(id, reason, memory) {
    ...
    const result = await this.handoffRouter.route({ conversationId: id, reason });
    ...
    if (result.status === "completed") {
      ...
      await this.memories.merge(id, { handoff, flujo: { ...(memory.flujo || {}), fase: "transferencia", siguiente_paso: null } });
  ...
  async process(conversationId, snapshot) {
    ...
    if (pendingHandoffMemory.handoff?.status === "pending" && !pendingHandoffMemory.handoff?.completed) {
      ...
      if (retryResult.status === "completed") {
        console.log(JSON.stringify({ event: "handoff_retry_completed", version: "3.2.1", conversationId }));
        return;
      }
    }
    ...
    if (Number(memory.experiencia?.frustration_score || 0) >= 2) {
      ...
      console.log(JSON.stringify({ event: "frustration_handoff", version: "3.2.1", conversationId, messageIds }));
      return;
    }
    ...
    if (orchestration.shouldHandoffB2B || memory.intent?.id === "PROVEEDOR") {
      ...
      console.log(JSON.stringify({ event: "b2b_handoff", version: "3.2.1", conversationId, messageIds }));
      return;
    }
    ...
    if (reason) {
      ...
      console.log(JSON.stringify({ event: "handoff", version: "3.2.1", conversationId, messageIds, reason, sources: snapshot.sources, memory }));
      return;
    }
    ...
    await this.memories.markProcessedMany(conversationId, messageIds);
    console.log(JSON.stringify({ event: "processed", version: "3.2.1", conversationId, messageIds, sources: snapshot.sources, planner, labels: currentLabels, memory: this.memories.get(conversationId) }));
  }
}
