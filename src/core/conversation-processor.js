...
export class ConversationProcessor {
  ...
  async process(conversationId, snapshot) {
    ...
    if (Number(memory.experiencia?.frustration_score || 0) >= 2) {
      ...
      console.log(JSON.stringify({ event: "frustration_handoff", version: "3.2.0", conversationId, messageIds }));
      return;
    }
    ...
    if (orchestration.shouldHandoffB2B || memory.intent?.id === "PROVEEDOR") {
      ...
      console.log(JSON.stringify({ event: "b2b_handoff", version: "3.2.0", conversationId, messageIds }));
      return;
    }
    ...
    if (reason) {
      ...
      console.log(JSON.stringify({ event: "handoff", version: "3.2.0", conversationId, messageIds, reason, sources: snapshot.sources, memory }));
      return;
    }
    ...
    await this.memories.markProcessedMany(conversationId, messageIds);
    console.log(JSON.stringify({ event: "processed", version: "3.2.0", conversationId, messageIds, sources: snapshot.sources, planner, labels: currentLabels, memory: this.memories.get(conversationId) }));
  }
}
