...
export class ConversationProcessor {
  ...
  async process(conversationId, snapshot) {
    ...
    const patience = analyzePatience(combinedText, memoryWithIntent);
    ...
    for (const event of patience.events) await this.record(conversationId, event.type, event);
    ...
    memory = mergeMemory(memoryWithIntent, llmPatch, reliability.patch, judgment.patch, patience.patch, { orchestration: { direct_request: judgment.question || orchestration.directRequest, direct_answer: preferredDirectAnswer }, judgment: judgment.patch.judgment });
    ...
    // V3.3.3: respetar pausas del cliente y cortar presión sobre CURP/NSS.
    ...
    if (directRequest && ["curp","nss"].includes(planner?.question_key)) {
      planner = { ...planner, action:"responder_pregunta_cliente", question_key:null, specialized:true, resume_after_answer:false };
    }
    // Guardia final: un slot sensible pausado/no disponible no puede volver a ser solicitado.
    if (["curp","nss"].includes(planner?.question_key) && sensitiveSlotSuppressed(memory, planner.question_key)) {
      planner = { action:"esperar_o_continuar_sin_dato_sensible", question_key:null, specialized:true, sensitive_guard:true };
    }
