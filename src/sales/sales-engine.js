...
export function planNext(memory){
  ...
  if(!memory.curp_recibida && !resolved(memory,"curp") && !slotBlocked(memory,"curp") && memory.slots?.curp_disponible!==false) {
    return {action:"solicitar_curp",question_key:"curp",rephrase:false};
  }
  if(!memory.nss_recibido && !resolved(memory,"nss") && !slotBlocked(memory,"nss") && memory.slots?.nss_disponible!==false) return {action:"solicitar_nss",question_key:"nss",rephrase:false};
