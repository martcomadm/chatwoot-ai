...
import { slotBlocked } from "../semantic/slot-governance.js";
import { sensitiveSlotSuppressed } from "../semantic/conversational-patience.js";
...
export function planNext(memory){
  ...
  if(!memory.curp_recibida && !resolved(memory,"curp") && !slotBlocked(memory,"curp") && !sensitiveSlotSuppressed(memory,"curp") && memory.slots?.curp_disponible!==false) {
    return {action:"solicitar_curp",question_key:"curp",rephrase:false};
  }
  if(!memory.nss_recibido && !resolved(memory,"nss") && !slotBlocked(memory,"nss") && !sensitiveSlotSuppressed(memory,"nss") && memory.slots?.nss_disponible!==false) return {action:"solicitar_nss",question_key:"nss",rephrase:false};
  if(memory.data_collection?.sensitive_requests_suppressed || ["unavailable","promised_later","searching","declined","ask_later","refused"].includes(memory.slots?.curp_estado)) return {action:"esperar_o_continuar_sin_dato_sensible",question_key:null,specialized:true};
  if(memory.slots?.curp_disponible===false && memory.slots?.nss_disponible===false) return {action:"transferir_datos_no_disponibles",question_key:null,specialized:true};
