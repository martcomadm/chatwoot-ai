...
export function createOperationsRouter({ config, saleStore, workflow }) {
  ...
  router.get("/operations/api/sales/:id",guard,(req,res)=>{const sale=saleStore.get(req.params.id);if(!sale)return res.status(404).json({error:"Expediente no encontrado"});res.json({sale});});
