...
export function createOperationsRouter({ config, saleStore, workflow }) {
  ...
  router.get("/operations/sales/:id",(_req,res)=>res.type("html").send(saleDetailPage(_req.params.id)));
  router.get("/operations/api/sales",guard,(req,res)=>{
    let items=saleStore.list({queue:req.query.queue||undefined,status:req.query.status||undefined});
    ...
    if(search)items=items.filter(s=>[s.sale_id,s.customer?.nombre,s.customer?.telefono,s.customer?.curp,s.customer?.nss,s.conversation_id].some(v=>String(v||"").toLowerCase().includes(search)));
    res.json({items});
  });
