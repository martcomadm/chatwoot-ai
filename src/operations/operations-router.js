...
export function createOperationsRouter({ config, saleStore, workflow }) {
  ...
  function authorized(req) {
    ...
    return req.get("x-operations-token") === expected || req.query.token === expected;
  }

  function guard(req, res, next) {
    if (!authorized(req)) return res.status(401).json({ error: "Token de Operations inválido" });
    next();
  }

  function action(handler) {
    return (req, res) => {
      try { return res.json({ ok: true, sale: handler(req) }); }
      catch (error) { return res.status(400).json({ error: error.message }); }
    };
  }

  router.get("/operations", (_req, res) => res.type("html").send(operationsPage()));
  router.get("/operations/api/sales", guard, (req, res) => res.json({ items: saleStore.list({ queue: req.query.queue || undefined, status: req.query.status || undefined }) }));
  router.get("/operations/api/sales/:id", guard, (req, res) => {
  ...
  router.get("/operations/api/events", guard, (req, res) => {
