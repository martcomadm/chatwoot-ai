...
export function analyzeSales(memory){
  ...
  const plan=(interests.infonavit||interests.afore||memory.necesidad_principal==="plan_2")?"plan_2":
    (interests.servicio_medico||memory.necesidad_principal==="servicio_medico")?"plan_1":null;
...
export function planNext(memory){
  if((memory.contradicciones||[]).length) return {action:"aclarar_contradiccion",question_key:"aclarar_contradiccion"};
  if(!present(memory.necesidad_principal) && !Object.values(memory.intereses||{}).some(Boolean)) return {action:"preguntar_necesidad",question_key:"necesidad_principal"};
  if(memory.tiene_imss===null || memory.tiene_imss===undefined) return {action:"preguntar_imss",question_key:"tiene_imss"};
  if(!present(memory.nombre)) return {action:"preguntar_nombre",question_key:"nombre"};
  if(!present(memory.edad)) return {action:"preguntar_edad",question_key:"edad"};
  if(!present(memory.actividad)) return {action:"preguntar_actividad",question_key:"actividad"};
  if(!memory.curp_recibida) return {action:"solicitar_curp",question_key:"curp"};
  if(!memory.nss_recibido) return {action:"solicitar_nss",question_key:"nss"};
