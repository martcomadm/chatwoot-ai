...
export function createRouter({ config, memories, buffer, inspectorEvents }) {
  ...
  router.get("/inspector/api/health", (req, res) => {
    ...
    res.json({ status: "ok", overall: diagnostics.overall, version: "3.1.2", inspectorVersion: "1.1.0", architecture: "modular", diagnostics });
  });
  ...
  router.get("/health", (_req, res) => res.json({ status: "ok", version: "3.1.2", timestamp: new Date().toISOString() }));
