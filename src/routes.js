...
export function createRouter({ config, memories, buffer, inspectorEvents, handoffRotation, operationsConfig }) {
  ...
  router.get("/inspector/api/health", (req, res) => {
    ...
    res.json({ status: "ok", overall: diagnostics.overall, version: "3.3.2.1", inspectorVersion: "1.5.2", architecture: "modular", lastEventAt: inspectorEvents.stats?.().lastEventAt || null, autoHandoff: config.handoff.enabled, diagnostics });
  });
