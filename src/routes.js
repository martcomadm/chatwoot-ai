...
export function createRouter({ config, memories, buffer, inspectorEvents, handoffRotation }) {
  ...
  router.get("/inspector/api/health", (req, res) => {
    ...
    res.json({ status: "ok", overall: diagnostics.overall, version: "3.2.1", inspectorVersion: "1.2.0", architecture: "modular", lastEventAt: inspectorEvents.stats?.().lastEventAt || null, autoHandoff: config.handoff.enabled, diagnostics });
  });
  router.get("/inspector/api/dashboard", (req, res) => {
    ...
    res.json({ stats: dashboardStats(all, events), filters: uniqueFilterOptions(all), handoffs: handoffMetrics(all), rotations: rotationOverview(config, handoffRotation) });
  });
  ...
  router.get("/inspector/api/conversations/:conversationId", (req, res) => {
    ...
    res.json({
      ...
      rotations: rotationOverview(config, handoffRotation),
    });
  });
  ...
  router.get("/health", (_req, res) => res.json({ status: "ok", version: "3.2.1", timestamp: new Date().toISOString() }));
