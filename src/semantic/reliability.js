...
import { governSlotAnswer } from "./slot-governance.js";
...
export function analyzeReliability(text, memory, fastPatch = {}) {
  ...
  const governed = governSlotAnswer(text, memory);
  const deterministicPatch = {
    ...
    slots: { ...(fastPatch.slots || {}), ...(answer.patch.slots || {}), ...(governed.patch.slots || {}) },
    blocked_questions: [...new Set([...(fastPatch.blocked_questions || []), ...(answer.patch.blocked_questions || []), ...(governed.patch.blocked_questions || [])])],
  };
