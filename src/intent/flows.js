...
function retirementByDeathFlow(memory) {
  ...
  if (data.afiliado_imss_al_fallecer == null) return { action: "preguntar_afiliacion_fallecido", question_key: "afiliado_imss_al_fallecer", rephrase: asked(memory, "afiliado_imss_al_fallecer"), specialized: true };
  if (data.afore_contactada == null) return { action: "preguntar_gestion_afore", question_key: "afore_contactada", rephrase: asked(memory, "afore_contactada"), specialized: true };
  if (!data.motivo_negativa && data.pension_negada) return { action: "preguntar_motivo_negativa", question_key: "motivo_negativa", rephrase: asked(memory, "motivo_negativa"), specialized: true };
  if (!Array.isArray(data.beneficiarios) || !data.beneficiarios.length) return { action: "preguntar_beneficiarios", question_key: "beneficiarios_fallecimiento", rephrase: asked(memory, "beneficiarios_fallecimiento"), specialized: true };
