...
export function buildDiagnostics({ config, memories, inspectorEvents }) {
  ...
  const components = [
    ...
    { name: "Auto Handoff", status: config.handoff.enabled ? "ok" : "warning", detail: config.handoff.enabled ? `Activo · Domingo ${config.handoff.sundayAgents.length} agente(s) · Sábado ${config.handoff.saturdayAgents.length} agente(s) · Entre semana ${config.handoff.weekdayAgents.length} agente(s).` : "Desactivado por configuración." },
