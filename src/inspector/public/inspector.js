...
function eventClass(e){if(['openai_error','processor_error','chatwoot_write_error','handoff_assignment_failed'].includes(e.type))return 'error';if(['chatwoot_read_fallback','quality_fallback','quality_repair','handoff_assignment_skipped'].includes(e.type))return 'warning';return ''}
function eventLabel(t){var m={buffer_flush:'Buffer procesado',chatwoot_read_fallback:'Fallback Chatwoot',intent_classified:'Intención clasificada',decision_state:'Planner',memory_updated:'Memoria actualizada',quality_checked:'Control de calidad',quality_repair:'Respuesta reparada',quality_fallback:'Fallback de calidad',ai_reply_sent:'Respuesta enviada',handoff:'Transferencia',handoff_summary_created:'Resumen creado',handoff_assignment_started:'Asignación iniciada',handoff_assignment_completed:'Asignación completada',handoff_assignment_failed:'Asignación fallida',handoff_assignment_skipped:'Asignación omitida',ignored_out_of_schedule:'Fuera de horario',ignored_no_usable_messages:'Sin mensaje utilizable'};return m[t]||t}
...
var h=m.handoff||{};var summary='<div id="tab-summary" class="tabpane"><div class="stats">'+['Intención|'+(i.label||i.id),'Confianza|'+pct(i.confidence),'Fase|'+f.fase,'Siguiente paso|'+f.siguiente_paso,'Asesor IA|'+m.asesor_presentacion,'Handoff|'+(h.status||'No iniciado')].map(function(x){var a=x.split('|');return '<div class="stat"><small>'+esc(a[0])+'</small><b>'+esc(a.slice(1).join('|'))+'</b></div>'}).join('')+'</div><div class="cols"><div class="panel"><h3>Perfil del cliente</h3><div class="inner">'+row('Nombre',m.nombre)+row('Edad',m.edad)+row('Actividad',m.actividad)+row('IMSS actual',m.tiene_imss===true?'Sí':m.tiene_imss===false?'No':'No informado')+row('Necesidad',m.necesidad_principal)+row('Última cotización',m.ultima_cotizacion)+row('AFORE',m.afore_actual)+'</div></div><div class="panel"><h3>Estado comercial</h3><div class="inner">'+row('Plan recomendado',s.plan_recomendado)+row('Temperatura',s.temperatura)+row('Problema',s.problema)+row('CURP recibida',m.curp_recibida?'Sí':'No')+row('NSS recibido',m.nss_recibido?'Sí':'No')+row('Contradicciones',(m.contradicciones||[]).length)+'</div></div><div class="panel"><h3>Handoff automático</h3><div class="inner">'+row('Estado',h.status)+row('Asesor real',h.agent_name)+row('ID Chatwoot',h.agent_id)+row('Turno',h.group)+row('Posición',h.rotation_position&&h.total_agents?(h.rotation_position+' / '+h.total_agents):h.rotation_position)+row('Asignado',h.assigned_at)+row('Último error',h.last_error)+'</div></div></div></div>';
