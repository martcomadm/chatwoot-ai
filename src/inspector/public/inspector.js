/* MARTCOM Inspector 1.5 - Dynamic Operations UI */
...
function setRange(kind){
  ...
  el('closeAdvisorModal').addEventListener('click',function(){el('advisorModal').classList.add('hidden')});
  el('advisorModal').addEventListener('click',function(event){
    if(event.target===el('advisorModal'))el('advisorModal').classList.add('hidden');
  });
...
function renderAdvisorModalList(agents){
  ...
  el('advisorModalList').innerHTML=filtered.map(function(a){
    return '<div class="advisor-pick-row"><div><b>'+esc(a.name)+'</b><small>#'+a.id+'</small></div>'+
      '<button data-add-existing="'+a.id+'">Agregar a '+CONTROL_STATE.activeTab.toUpperCase()+'</button></div>';
...
function bindUi(){
  ...
  el('controlBody').addEventListener('click',async function(event){
    ...
    if(toggleMenu){
      var row3=toggleMenu.closest('.dynamic-agent-row');
      var agent=currentGroupAgents().find(function(a){return Number(a.id)===Number(row3.dataset.agentId)});
      if(agent){
    ...
    if(event.target.id==='createException'){
      ...
      try{
        ...
        await adminApi('/inspector/api/control/exceptions/'+date,{method:'PUT',body:JSON.stringify({agents:baseAgents})});
        await loadControl();await refreshAll();
      }catch(e){alert(e.message)}
    }
  });
