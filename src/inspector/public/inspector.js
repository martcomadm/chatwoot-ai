/* MARTCOM Inspector 1.4.3 - Compact Agent Manager */
...
function agentEditor(group,agents){
  return '<div class="control-group" data-group="'+group+'">'+
    '<div class="control-group-head"><b>'+esc(group.toUpperCase())+'</b><span class="group-count">'+(agents||[]).length+' asesores</span><button data-save-group="'+group+'">Guardar</button></div>'+
    '<div class="agent-editor">'+(agents||[]).map(function(a,i){
      var destinations=['weekday','saturday','sunday'].filter(function(g){return g!==group});
      return '<div class="agent-edit compact-row" data-agent-id="'+a.id+'">'+
        '<span class="drag-index">'+(i+1)+'</span>'+
        '<div class="agent-name-wrap"><b>'+esc(a.name)+'</b><small>#'+a.id+'</small></div>'+
        '<label class="toggle-compact"><input type="checkbox" '+(a.enabled!==false?'checked':'')+'><span>Activo</span></label>'+
        '<div class="move-buttons"><button data-move="up">↑</button><button data-move="down">↓</button></div>'+
        '<select class="target-select" data-target-group>'+destinations.map(function(g){return '<option value="'+g+'">'+g.toUpperCase()+'</option>'}).join('')+'</select>'+
        '<button class="mini-action move-action" data-agent-move>Mover</button>'+
        '<button class="mini-action copy-action" data-agent-copy>Copiar</button>'+
      '</div>';
    }).join('')+'</div></div>';
}
...
function renderControl(d){
  ...
  var auditHtml=audit.length
    ? audit.slice(0,40).map(function(a){
        return '<div class="audit-row"><time>'+fmtDate(a.timestamp)+'</time><b>'+esc(a.type)+'</b><span>'+esc(a.details?.group||a.details?.targetGroup||a.details?.date||'')+'</span></div>';
      }).join('')
    : '<div class="empty compact">Sin cambios registrados.</div>';
  el('controlBody').innerHTML=
    '<div class="master-agents compact-master">'+
      '<div class="master-head"><div><h3>Asesores disponibles</h3><small>'+agents.length+' registrados · puedes agregar un usuario de Chatwoot por ID.</small></div>'+
      '<div class="add-agent-form"><input id="newAgentId" type="number" min="1" placeholder="ID"><input id="newAgentName" placeholder="Nombre del asesor"><button id="addMasterAgent">+ Agregar asesor</button></div></div>'+
      '<div class="master-grid compact-grid">'+agents.map(function(a){
        return '<div class="master-agent compact-master-agent"><div><b>'+esc(a.name)+'</b><small>#'+a.id+'</small></div>'+
    ...
    '</div>'+
    '<div class="control-grid compact-control-grid">'+agentEditor('weekday',groups.weekday)+agentEditor('saturday',groups.saturday)+agentEditor('sunday',groups.sunday)+'</div>'+
    ...
    '<div class="audit-panel"><h3>Historial de cambios</h3>'+auditHtml+'</div>';
}
...
function bindUi(){
  ...
  el('controlBody').addEventListener('click',async function(event){
    ...
    if(event.target.id==='addMasterAgent'){
      try{await addMasterAgent()}catch(e){alert(e.message)}
      return;
    }
