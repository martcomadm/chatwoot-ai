/* MARTCOM Inspector 1.5.1 - Modal Stack & Delete Fix */
...
function bindUi(){
  ...
  el('controlBody').addEventListener('click',async function(event){
    ...
    if(removeBtn){
      var removeRow=removeBtn.closest('.dynamic-agent-row');
      try{await removeFromCurrentGroup(removeRow.dataset.agentId)}catch(e){alert(e.message)}
      return;
    }
