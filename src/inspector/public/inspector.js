/* MARTCOM Inspector 1.4.1 - Control Modal Fix */
...
function agentEditor(group,agents){
  return '<div class="control-group" data-group="'+group+'"><div class="control-group-head"><b>'+esc(group.toUpperCase())+'</b><button data-save-group="'+group+'">Guardar</button></div><div class="agent-editor">'+
    (agents||[]).map(function(a,i){return '<div class="agent-edit" data-agent-id="'+a.id+'"><span class="drag-index">'+(i+1)+'</span><b>'+esc(a.name)+'</b><small>#'+a.id+'</small><label class="switch-label"><input type="checkbox" '+(a.enabled!==false?'checked':'')+'> Activo</label><div class="move-buttons"><button data-move="up">↑</button><button data-move="down">↓</button></div></div>'}).join('')+
    '</div></div>';
}
...
function renderControl(d){
  var groups=d.groups||{}, exceptions=d.exceptions||{}, audit=d.audit||[];
  el('controlBody').innerHTML='<div class="control-grid">'+agentEditor('weekday',groups.weekday)+agentEditor('saturday',groups.saturday)+agentEditor('sunday',groups.sunday)+'</div>'+
    ...
    '<div class="audit-panel"><h3>Historial de cambios</h3>'+(audit.length?audit.slice(0,40).map(function(a){return '<div class="audit-row"><time>'+fmtDate(a.timestamp)+'</time><b>'+esc(a.type)+'</b><span>'+esc(a.details?.group||a.details?.date||'')+'</span></div>'}).join(''):'<div class="empty compact">Sin cambios registrados.</div>')+'</div>';
}
