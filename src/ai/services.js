...
export class AiServices {
  ...
  async extractDocumentData(attachments = []) {
    const images=(attachments||[]).filter(a=>String(a?.file_type||a?.content_type||"").toLowerCase().includes("image")).map(a=>a?.data_url||a?.download_url||a?.file_url||a?.url).filter(Boolean).slice(0,3);
