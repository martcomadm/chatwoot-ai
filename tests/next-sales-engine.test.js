...
test("explicit process authorization phrase is detected", () => { assert.equal(detectAuthorization("Sí, quiero iniciar el trámite"), true); assert.equal(detectAuthorization("Adelante con el proceso"), true); });
...
test("authorization persists selected plan after commercial context", () => { let m=apply({},"Me interesa AFORE e Infonavit");m=apply(m,"Sí, quiero iniciar el trámite");assert.equal(m.sales_cycle.authorized,true);assert.equal(m.sales_cycle.selected_plan,"plan_2"); });
test("quiero darme de alta while exploring is interest, not operational authorization", () => { const r=analyzeNextSale("Quiero darme de alta",{});assert.equal(r.patch.sales_cycle.interested,true);assert.equal(r.patch.sales_cycle.authorized,false);assert.equal(r.patch.sales_cycle.stage,"interested"); });
test("explicit authorization without a prior plan or proposal does not open expediente", () => { const r=analyzeNextSale("Sí, quiero iniciar el trámite",{});assert.equal(r.patch.sales_cycle.authorized,false);assert.equal(r.patch.sales_cycle.stage,"interested"); });
test("explicit authorization after recommendation opens operations", () => { const r=analyzeNextSale("Sí, quiero iniciar el trámite",{sales_cycle:{stage:"plan_recommended",recommended_plan:"plan_1"}});assert.equal(r.patch.sales_cycle.authorized,true);assert.equal(r.patch.sales_cycle.stage,"authorized");assert.equal(r.patch.sales_cycle.selected_plan,"plan_1"); });
