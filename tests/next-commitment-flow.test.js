...
test("explicit process start after commercial context authorizes", () => {
  ...
  const decision = commitmentDecision(memory, "Sí, quiero iniciar el trámite");
...
test("already-authorized state is handed off to operations onboarding", () => {
  const authorized = { sales_cycle: { stage: "authorized", recommended_plan: "plan_1", selected_plan: "plan_1", authorized: true } };
  const decision = commitmentDecision(authorized, "Sí, quiero iniciar el trámite");
