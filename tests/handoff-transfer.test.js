...
test("transferencia crea resumen antes de asignar al asesor humano", async () => {
  ...
  await processor.transfer(99, {}, "CURP recibida", { flujo: { fase: "diagnostico" }, advisor_affinity: { agent_id: 25, agent_name: "Elizabeth Aguilera", group: "sunday", rotation_position: 1, total_agents: 3 } });
  ...
  assert.equal(stored.advisor_affinity.agent_id, 25);
  assert.equal(stored.advisor_affinity.status, "assigned");
});
