...
test("si Chatwoot falla, la rotación NO avanza", async () => {
  const { router } = setup({ failIds: new Set([25]) });
  const first = await router.route({ conversationId: 301, reason: "test", date: sunday });
  const second = await router.route({ conversationId: 302, reason: "test", date: sunday });
  assert.equal(first.status, "failed");
  assert.equal(second.status, "failed");
  assert.equal(first.agent.id, 25);
  assert.equal(second.agent.id, 25);
});
