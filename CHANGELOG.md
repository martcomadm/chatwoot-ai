...
- Respeta "mañana te lo mando", "espérame", "lo estoy buscando", "no me sé su CURP".
- Preguntas directas tienen prioridad y no terminan inmediatamente en CURP/NSS.
- CURP deja de funcionar como fallback universal.
- Protección adicional de nombre para frases que comienzan con "Sería".
- Se conserva Inspector V1.6 Analytics & Forecasting.
- Corrige referencia residual `joined` en Negation Scope.

## Core V3.3.2.1 — Negation Scope Hotfix
- Corrige ReferenceError: joined is not defined.
- El Negation Scope Resolver usa el texto entrante real disponible.
- Inspector V1.5.2 sin cambios.

## Core V3.3.2 — Negation Scope Resolver
- Evita falsos rechazos por falta de comas.
- Ambigüedad obliga confirmación.
- Rechazos claros siguen funcionando.
...
- Restablece drag & drop, mover, copiar, quitar y eliminar.
- Agrega alta manual por ID + nombre.

## Inspector V1.5.1 — Modal Stack & Delete Fix
- Corrige z-index de Agregar asesor.
- X, fondo y Escape cierran el modal secundario.
- Quitar asesor de un turno.
- Eliminar asesor del catálogo con protecciones.

## Inspector V1.5 — Dynamic Operations UI
- Pestañas por turno.
- Drag & drop.
- Menú contextual por asesor.
- Buscador y modal de agregar asesor.
- Excepciones e historial en vistas separadas.
